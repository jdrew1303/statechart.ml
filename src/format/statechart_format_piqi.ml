...
and parse_document x =
  ...
  let _initial, x =  Piqirun.parse_flag 4 x in
...
and gen__document code x =
  ...
  let _initial =  Piqirun.gen_flag 4 x.Document.initial in
  let _states = Piqirun.gen_repeated_field 5 gen__state x.Document.states in
  Piqirun.gen_record code (_name :: _binding :: _datamodel :: _initial :: _states :: [])
